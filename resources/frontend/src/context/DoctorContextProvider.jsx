...
const DoctorContextProvider = (props) => {
    ...
    const updateProfile = async () => {
        try {
            ...
            if (profileData.fees)       formData.append('fees',        profileData.fees)
            if (profileData.available !== undefined) formData.append('is_approved', profileData.available ? 1 : 0)
            if (profileData.about)      formData.append('bio',         profileData.about)
            if (profileData.address?.line1) formData.append('address', profileData.address.line1)

            const { data } = await axios.put(backendUrl + '/api/doctor/updateProfile', formData, {
